...
namespace comfoair {
  ...
  void ComfoAir::setup() {
    ...
    // TODO matt.stasch: Remove!
    mqtt->subscribeTo(MQTT_PREFIX "/commands/" "ventilation_level",
      [this](char const* _1, uint8_t const* _2, int _3) {
    ...
    // TODO matt.stasch: Remove!
    mqtt->subscribeTo(MQTT_PREFIX "/commands/" "set_mode",
      [this](char const* _1, uint8_t const* _2, int _3) {
...
#define ON(expected) if (memcmp(expected, payload, min((int)sizeof(expected), length)) == 0)
    ...
    mqtt->subscribeTo(MQTT_PREFIX "/climate/fan/set",
      [this](char const* topic, uint8_t const* payload, int length)
      {
    ...
    mqtt->subscribeTo(MQTT_PREFIX "/climate/mode/set",
      [this](char const* topic, uint8_t const* payload, int length) {
        // Handle HA Fan message
        ON("auto") {
    ...
    mqtt->subscribeTo(MQTT_PREFIX "/climate/preset/set",
      [this](char const* topic, uint8_t const* payload, int length) {
        ON("auto") {
